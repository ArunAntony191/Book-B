...
try {
    ...
    if ($stmt->execute([$token, $expiry, $email])) {
        ...
        header("Location: ../pages/verify_email.php?email=" . urlencode($email) . ($mailSent ? "" : "&token=" . $token . "&not_sent=1"));
    } else {
        header("Location: ../pages/forgot_password.php?error=failed");
    }
    
} catch (PDOException $e) {
    // Show readable error for debugging if needed
    $errorMsg = urlencode($e->getMessage());
    header("Location: ../pages/forgot_password.php?error=failed&debug=" . $errorMsg);
}
exit();
?>
